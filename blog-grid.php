...
<body id="body">
					...
					<nav class="navbar navbar-expand-lg p-0">
						...
						<div class="collapse navbar-collapse ml-auto" id="navbarsExample09">
							<ul class="navbar-nav ml-auto">
								...
								<li class="nav-item active"><a class="nav-link" href="areariservata.html"><img src="images/chiave.png">Area Riservata</a></li>
	...
	<div class="page-wrapper">
		<div class="container">
			<div class="row">
				<div class="col-12">
					<table class="table custom-table">
						...
						<?php
						$user = $_GET['utente'];
						$nomeserver = "localhost"; //lato server si può usare anche "localhost"
						$nomeutente = "geotanzi";
						$password = "";
						$database = "my_geotanzi";
						$connessione = mysqli_connect($nomeserver, $nomeutente, $password);
						$query = "SELECT * FROM my_geotanzi.posizione, my_geotanzi.superutente, my_geotanzi.utente WHERE superutente.utentecapo = '$user' AND posizione.nome = utente.nome AND utente.nome = superutente.utentefiglio ORDER BY Data DESC";
						$risultato = mysqli_query($connessione, $query) or die(mysqli_error($connessione));
						$quanti = mysqli_num_rows($risultato);
						if ($quanti == 0) {
							echo "Nessun record!";
						} else {
							for ($x = 0; $x < $quanti; $x++) {
								$rs = mysqli_fetch_row($risultato);
								$lat = $rs[0];
								$long = $rs[1];
								$nom = $rs[2];
								$data = $rs[3];
						?>
								<tbody>
									<tr>
										...
										<td> <?php echo $data ?> </td>
										<td>
											<a href="https://www.google.com/maps/search/?api=1&query= <?php echo $lat ?>, <?php echo $long ?>">Maps</a>
										</td>
										<td>
									<button type="button" class="btn btn-primary" onclick="vedimappa.alert('Identificativo del tuo dispositivo!')"> <i class="far fa-eye"></i></button>
									<button type="button" class="btn btn-danger" onclick="elimina()"><i
			...
			<button class="btn btn-danger" onclick="exitarea()">Esci</button>
		</div>
	</div>
